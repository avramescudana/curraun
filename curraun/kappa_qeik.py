...
def compute_f(s, f, stream):
    ...
    my_parallel_loop(compute_f_kernel, n * n, aeta0, f, tau, stream=stream)

@myjit
def compute_f_kernel(xi, aeta0, f, tau):
    #### tildeEx = tildeEy = 0

    for d in range(2):
        su.store(f[xi, d], su.zero())

    #### tildeEz = - A\eta / \tau^2
    bf = su.mul_s(aeta0[xi], - 1.0 / (tau * tau))
    su.store(f[xi, 2], bf)
