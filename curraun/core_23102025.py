...
class Simulation:
    def __init__(self, n, dt, g):
        ...
        """                         Start: Pooja's comments                     """
        # N꜀ = 2                 # Number of colors, SU(N꜀ = 2)
        # ALGEBRA_ELEMENTS = 3
        # GROUP_ELEMENTS = 4
        # CASIMIRS = 1

        # N꜀ = 3                 # Number of colors, SU(N꜀ = 3)
        # ALGEBRA_ELEMENTS = 8
        # GROUP_ELEMENTS = 9
        # CASIMIRS = 2

        # U_{x,i}(tau_n) = u0           and         U_{x,i}(tau_(n+1)) = u1
        # At each of the Nₜ² lattice sites
        # For each of the 2 spatial directions (x and y)
        # Nc × Nc matrix for SU(Nc)

        # Lattice: Nₜ × Nₜ sites
        # At each site (i,j):
        #     |
        #     +-- Direction 0 (x): [N꜀×N꜀ matrix for SU(N꜀)]
        #     +-- Direction 1 (y): [N꜀×N꜀ matrix for SU(N꜀)]


        """                          End: Pooja's comments                      """
