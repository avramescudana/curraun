...
"""
    A module to perform the LC gauge transformation of the Glasma fields at each x^+ slice taking t = tau
"""
...
class LCGaugeTransf:
    ...
    def evolve_lc(self, xplus):
        ...
        for xminus in range(0, -xplus+1, -1):
            compute_vlc(self.d_ux, self.d_aeta, self.n, xplus, xminus, self.d_vlc_dag)
        ...
        compute_uplus_temp(self.d_ux, self.d_aeta, self.n, xplus, self.d_up_temp)
        
        # We reorder the transformed fields
        reorder(self.d_up_lc_reorder, self.d_up_lc, self.n)
        reorder(self.d_up_temp_reorder, self.d_up_temp, self.n)
...
"""
def compute_uplus_temp(ux, aeta, n, xplus, up_temp):
    my_parallel_loop(compute_uplus_temp_kernel, n**2, ux, aeta, n, xplus, up_temp)
    
@myjit
def compute_uplus_temp_kernel(yi, ux, aeta, n, xplus, up_temp):
    ...
    if xplus == 0:
        ...
        tauxy_x = l.get_index_n2xm(xplus, xplus, y, n)
        tauxy_t = l.get_index_n2xm(xplus, xplus+1, y, n)
        
        # We compute the x and t links
        ux_latt = ux[tauxy_x]
        
        # aeta_latt = aeta[tauxy_t]
        # aeta_fact = su.mul_s(aeta_latt, -(z-n//2)/xplus**2)
...
"""
    Compute the gauge transformation operator
"""
def compute_vlc(ux, aeta, n, xplus, xminus, vlc_dag):
    my_parallel_loop(compute_vlc_kernel, n**2, ux, aeta, n, xplus, xminus, vlc_dag)
    
@myjit
def compute_vlc_kernel(yi, ux, aeta, n, xplus, xminus, vlc_dag):
    ...
    tauxy = l.get_index_n2xm(xplus+xminus-1, xplus-xminus, y, n)
            
    # Compute the x and t links
    ux_latt = ux[tauxy]
            
    # aeta_latt = aeta[tauxy]
    # aeta_fact = su.mul_s(aeta_latt, -(z-n//2)/(xplus+xminus-1)**2)
    # ut = su.mexp(aeta_fact)
