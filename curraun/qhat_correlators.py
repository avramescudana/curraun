...
class TransverseTransportedForce:
    """
    Mirror image of LongTransportedForce: keeps only the transverse-field
    pieces of the force, E_y (f_2) and B_y (f_3), dropping -B_z and E_z.
    """
    def __init__(self, s):
        self.s = s
        self.n = s.n
    ...
    def compute(self, stream=None):
        tint = round(self.s.t / self.s.dt)
        if tint % self.dtstep == 0 and tint >= 1:
            # compute un-transported f (transverse-field-only)
            compute_f_trans(self.s, self.d_f, round(self.s.t - 10E-8), stream)
...
"""
    Transverse-field-only variant of compute_f: keeps E_y (f_2) and B_y (f_3),
    drops the longitudinal -B_z and E_z pieces.
"""


def compute_f_trans(s, f, t, stream):
    u0 = s.d_u0
    pt1 = s.d_pt1
    aeta0 = s.d_aeta0
    pt0 = s.d_pt0
    ...
    my_parallel_loop(compute_f_trans_kernel, n * n, n, u0, aeta0, pt1, pt0, f, t, tau, stream=stream)

@myjit
def compute_f_trans_kernel(xi, n, u0, aeta0, pt1, pt0, f, t, tau):

    # f_1 = E_1 (index 0), unchanged
    ...
    # f_2 = E_2 (index 1), transverse-only: drop -B_z
    ...
    # f_3 = B_2 (index 2), transverse-only: drop +E_z
    bf2 = su.zero()
