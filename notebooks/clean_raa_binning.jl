...
# ╔═╡ 6606ddde-b84a-11ee-3e8e-172a3efd55ff
begin
	using Pickle
	using NPZ
	using DataFrames
...
# ╔═╡ 510a081e-e854-475f-9208-e9d2b18fc777
md"### Import packages"

# ╔═╡ 9262e03b-8347-4841-9e18-dc9e7aa17097
md"---
### Histograms"

# ╔═╡ bfe9d733-4418-41ef-92b4-33e34f726392
begin
...
# ╔═╡ cf1453c2-ff57-4c5a-b68e-2f9e084f609d
md"---
### Choose type of binnning"

# ╔═╡ 5b32a552-290f-4f69-acc0-a24417ca36fd
binning = "pT2"
...
md"### Read Glasma results from files
"
...
# ╔═╡ 64a27525-9f61-498a-92e3-17415a986a82
begin
...
# ╔═╡ a086845c-a718-48f7-bade-de6328e5cbb7
begin
	collect_pTs = zeros(0)
...
# ╔═╡ 63abf6a0-93c6-4ec4-8a35-ba1b771c3eee
md"---
### Plot Glasma spectra"

# ╔═╡ 50ce7584-bd77-43fb-b6f9-4fbf905f3916
md"---
##### Initial distribution binned in $p_T^2$"

# ╔═╡ 8fcff179-bee9-4b82-8851-baa2e9c92a56
begin
	# just glasma, no weights
	pt2₀, dndpt2₀ = histogram(collect_initial_pTs, bin_edges, norm_hist)
	pt2₁, dndpt2₁ = histogram(collect_pTs, bin_edges, norm_hist)
end

# ╔═╡ 4c7424ca-75bc-46ab-8e72-cc6e2f8fdbbf
function complete_hist(x, y, bin_edges)
	x_hist, y_hist = copy(x), copy(y)
	prepend!(x_hist, bin_edges[1])
	prepend!(y_hist, y_hist[1])
	append!(x_hist, bin_edges[end])
	append!(y_hist, y_hist[end])
	return x_hist, y_hist
end

# ╔═╡ aa3d5686-87f2-4fc1-989f-d31352fc4661
begin
	# add one more value to make the first bin go to 0, otherwise stairs! plots it starting from the bin center
	pt2₀_hist, dndpt2₀_hist = complete_hist(pt2₀, dndpt2₀, bin_edges)
	pt2₁_hist, dndpt2₁_hist = complete_hist(pt2₁, dndpt2₁, bin_edges)
end

# ╔═╡ 020ad8db-617c-46da-8d20-04c4190c7d47
colors = Makie.wong_colors();

# ╔═╡ e815aa37-80b8-4897-82a8-4795bf43eace
begin
	set_theme!(fonts = (; regular = "CMU Serif"))
	fig_dndpt2_glasma = Figure(size = (350, 300), font = "CMU Serif")
	
	ax_dndpt2_glasma = Axis(fig_dndpt2_glasma[1,1], xlabel=L"p_T^2", ylabel=L"\mathrm{d}N/\mathrm{d}p_T^2", xlabelsize = 20, ylabelsize= 20, xticklabelsize=14, yticklabelsize=14, xgridvisible = false, ygridvisible = false) 

	plot_dndpt2₀ = stairs!(ax_dndpt2_glasma, pt2₀_hist, dndpt2₀_hist, step=:center, color=colors[5])
	plot_dndpt2₁ = stairs!(ax_dndpt2_glasma, pt2₁_hist, dndpt2₁_hist, step=:center, color=colors[4])

	# Histograms from Makie give the same result as "manually" extracted histograms but with these I don't have access to the resulting x-y values!
	# dndpt₀ = stephist!(ax_dndpt_glasma, collect_initial_pTs, bins=bin_edges, color=colors[5], normalization = norm_hist) 
	# dndpt₁ = stephist!(ax_dndpt_glasma, collect_pTs, bins=bin_edges, color=colors[4], normalization = norm_hist)

	axislegend(ax_dndpt2_glasma, [plot_dndpt2₀, plot_dndpt2₁], [L"\delta\tau=0\,\mathrm{fm/c}", L"\delta\tau=0.3\,\mathrm{fm/c}"], position = :lb, labelsize=14)

	xlims!(ax_dndpt2_glasma, 0, 100)
	# ylims!(ax_dndpt_glasma, 2000, 2900)

	save("plots/dNdpT2_glasma_"*gauge_group*"_"*representation*"_hist_"*binning*"_binning.png", fig_dndpt2_glasma, px_per_unit = 5.0)

	fig_dndpt2_glasma
end

# ╔═╡ c5f234b2-2f4c-4314-8737-3251dd1863b4
md"---
##### Convert histogram in $p_T^2$ to binning in $p_T$" 

# ╔═╡ fd43f425-14de-426f-8fa2-b4e42ebaaaff
bin_edges

# ╔═╡ d05f67be-e744-4477-a0c8-27e9f185d7ff
# Function to transform N(p^2) histogram to N(p) histogram
function transform_Np2_to_Np(p2_bin_edges, Np2_values)
    # Calculate bin widths for p^2
    p2_bin_width = diff(p2_bin_edges)[1]

    # Calculate bin edges for p
    p_bin_edges = sqrt.(p2_bin_edges)

    # Calculate bin midpoints for p
    p_bin_centers = [(p_bin_edges[i] + p_bin_edges[i+1])/2 for i in 1:length(p_bin_edges)-1]

    # Calculate transformation factor
    transformation_factor = 2 *  p_bin_centers

    # Calculate N(p) values
    Np_values = Np2_values .* transformation_factor .* p2_bin_width
    
    return p_bin_centers, p_bin_edges, Np_values
end

# ╔═╡ 3ab9fbb4-4397-42f1-970c-f0aee61b072f
begin
	pt₀, pt₀_edges, dndpt₀ = transform_Np2_to_Np(bin_edges, dndpt2₀)
	pt₁, pt₁_edges, dndpt₁ = transform_Np2_to_Np(bin_edges, dndpt2₁)

	pt₀_hist, dndpt₀_hist = complete_hist(pt₀, dndpt₀, pt₀_edges)
	pt₁_hist, dndpt₁_hist = complete_hist(pt₁, dndpt₁, pt₁_edges)
end

# ╔═╡ eac3cb19-5bee-41e1-9277-8b8d5c735422
begin
	set_theme!(fonts = (; regular = "CMU Serif"))
	...
	save("plots/dNdpT_glasma_"*gauge_group*"_"*representation*"_hist_"*binning*"_binning.png", fig_dndpt_glasma, px_per_unit = 5.0)

	fig_dndpt_glasma
end

# ╔═╡ 925a1bd0-e9a5-47b5-8101-f893219b06f5
md"# FONLL"

# ╔═╡ f72acd6b-8085-4d10-8658-fc3aff243db3
md"---
### Choose FONLL spectra parameters"

# ╔═╡ a9d25377-2396-4527-be69-2d14b52ae202
begin
	energy = 5500
	pdf = "cteq"
	quark = "charm"
...
# ╔═╡ 2a9473e3-c888-47f2-89d6-b48d80f58201
md"---
### Read FONLL spectra from files"

# ╔═╡ 5869c755-f320-4aa0-97f9-9eb49dbfc75c
begin
...
# ╔═╡ 73f5b938-14d5-4b98-b89e-16379a3a2ed0
md"---
### Interpolate FONLL spectra"

# ╔═╡ e1a46081-c5ca-4531-86a5-0294b8ea3e9c
begin
...
# ╔═╡ 7e62a5ca-fef1-4472-bd34-2db5703ba9b5
begin
	equal_pt_bin_edges = range(0, ptmax, nbins_hist+1)
	xdata_downsample = range(0, xdata_slice[end], 30)

	# bin centers for data
	xdata_slice_center = [(xdata_slice[i+1] + xdata_slice[i])/2 for i in 1:length(xdata_slice)-1]

	xdata_downsample_center = [(xdata_downsample[i+1] + xdata_downsample[i])/2 for i in 1:length(xdata_downsample)-1]
end

# ╔═╡ b6767959-5c51-4ff5-9df1-39fd01620786
begin
	set_theme!(fonts = (; regular = "CMU Serif"))
	...
	lines!(ax_fonll, equal_pt_bin_edges, interp_map(equal_pt_bin_edges), color=colors[1])
	scatter!(ax_fonll, xdata_slice, ydata_slice./integrate(xdata_slice, ydata_slice), color=colors[4], markersize=6)
	# scatter!(ax_fonll, xdata_slice_center, interp_map(xdata_slice_center), color=colors[2], markersize=6)
	stairs!(ax_fonll, xdata_downsample, interp_map(xdata_downsample), step=:center, color=colors[2])
	save("plots/dNdpT_FONLL_interp_hist_binning.png", fig_fonll, px_per_unit = 5.0)
...
md"---
### Bin FONLL spectra in $p_T^2$"

# ╔═╡ c958cf26-66f1-4de8-a852-18f5b8d0a9b0
# Function to transform N(p) histogram to N(p^2) histogram
function transform_Np_to_Np2(p_bin_edges, Np_values)
    # Calculate bin widths for p
    p_bin_width = diff(p_bin_edges)[1]

	# Calculate bin center for p
    p_bin_centers = [(p_bin_edges[i] + p_bin_edges[i+1])/2 for i in 1:length(p_bin_edges)-1]

    # Calculate bin edges for p^2
    p2_bin_edges = p_bin_edges .^ 2

    # Calculate bin midpoints for p^2
    p2_bin_centers = [(p2_bin_edges[i] + p2_bin_edges[i+1])/2 for i in 1:length(p2_bin_edges)-1]

    # Calculate transformation factor
    transformation_factor = 1 ./ (2 *  p_bin_centers)

    # Calculate N(p^2) values
    Np2_values = Np_values .* (transformation_factor .* p_bin_width)
    
    return p2_bin_centers, p2_bin_edges, Np2_values
end
...
begin
	...
	save("plots/dNdpT2_FONLL_interp_hist_binning.png", fig_fonll_pt2, px_per_unit = 5.0)
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "c7cb4fc14ff32a353812c48025054e65398f2cb6"
