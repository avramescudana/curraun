...
begin
	results = Pickle.npyload("mom_broad_q2_dep.pickle")
	q2s = results["q2s"]
...
# 	fig = Figure(resolution = (1000, 800), font = "CMU Serif")
...
# 	labels_q2 = ["0", "1/3", "2/3", "1", "4/3", "5/3", "2", "7/3", "8/3", "3", "10/3", "11/3", "4", "13/3", "14/3", "5"]
...
# 	save("plots/mom_broad_q2_dep.png", fig, px_per_unit = 5.0)
# 	fig
# end
...
begin
	...
	fig = Figure(resolution = (1000, 800), font = "CMU Serif")
	titles = [L"m\rightarrow\infty", L"m=m_\mathrm{beauty}", L"m=m_\mathrm{charm}"]
	ylabels = [L"\langle\delta p^2_T\rangle/q_2\,\mathrm{[GeV^2]}", L"\langle\delta p^2_L\rangle/q_2\,\mathrm{[GeV^2]}", L"\mathrm{ratio}"]
	axes = [Axis(fig[j, i], 
		xlabel=L"\delta\tau\,\mathrm{[fm/}c\mathrm{]}", ylabel=ylabels[j], xlabelsize = 22, ylabelsize= 22, xticklabelsize=16, yticklabelsize=16, xtickalign = 1, xticksize=4, ytickalign=1, yticksize=4, title= j == 1 ? titles[i] : "", titlesize=22
	) for i in 1:3, j in 1:3]
	...
	save("plots/mom_broad_div_q2_q2_dep.png", fig, px_per_unit = 5.0)
	fig
end
...
begin
	...
	save("plots/mom_broad_casimir_scaling.png", fig_cs, px_per_unit = 5.0)
	fig_cs
end
