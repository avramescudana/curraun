...
# ╔═╡ 4198ed34-f33f-11ed-36ac-c1f9962c07dc
begin
	using Pickle
	using CairoMakie
	using ColorSchemes
...
# ╔═╡ 1a8732f3-15be-445c-8b8e-fa7888359322
function string_as_varname(s::AbstractString,v::Any)
    s=Symbol(s)
    return @eval (($s) = ($v))
end

# ╔═╡ b1d2e8e4-4340-4933-b6ce-412e835ae338
quark = "beauty"

# ╔═╡ 2ab7978c-5e98-4c5c-b1ba-d5ce999b8f55
saveplots = true

# ╔═╡ 40cb386f-c93a-4ff0-9677-6452fcbdcac7
# [Gev * fm]
hbarc = 0.197326 

# ╔═╡ 559b2d7c-3184-442f-80be-6063fe08b931
rescaled = false

# ╔═╡ 066b7c42-cb52-43e0-a9d1-0068717b75a8
md"""
---
...
begin
	...
	file = current_path*"/results/Qs_mom_broad_wong_eikonal_jets.pickle"
...
begin
	lens = 120
	δτ_sel, ind_sel =  Dict(), Dict()
	for (iq, quark) in enumerate(quarks)
		for (iQs, Qs) in enumerate(Qss)
			tag = quarks[iq] * "_Qs_" * Qss[iQs]
			δτ = Float64.(tau_wong[tag])
			δτ_sel[tag] = LinRange(δτ[1],δτ[length(δτ)],lens)
...
begin
	...
	# fig = Figure(resolution = (1050, 500), font = "CMU Serif")
	# 	ylabels = reshape([L"\langle\delta p_L^2\,\rangle\,\mathrm{[GeV^2]}", L"\langle\delta p_T^2\,\rangle\,\mathrm{[GeV^2]}", L"\langle\delta p^2_L\,\rangle/\langle\delta p^2_T\,\rangle", L"\langle\delta p_L^2\rangle/Q_s^2", L"\langle\delta p_T^2\,\rangle/Q_s^2", L"\langle\delta p^2_L\,\rangle/\langle\delta p^2_T\,\rangle"], 3, 2)
	# 	xlabel = [L"\delta\tau\,\mathrm{[fm/c]}",L"Q_s\,\delta\tau"] 
	fig = Figure(resolution = (1050, 500), font = "CMU Serif")
		ylabels = reshape([L"\langle\delta p_z^2\,\rangle\,\mathrm{[GeV^2]}", L"\langle\delta p_y^2\,\rangle\,\mathrm{[GeV^2]}", L"\langle\delta p^2_z\,\rangle/\langle\delta p^2_y\,\rangle", L"\langle\delta p_z^2\rangle/Q_s^2", L"\langle\delta p_y^2\,\rangle/Q_s^2", L"\langle\delta p^2_z\,\rangle/\langle\delta p^2_y\,\rangle"], 3, 2)
		xlabel = [L"\tau\,\mathrm{[fm/c]}",L"Q_s\,\tau"] 
	
	axes = reshape([Axis(fig[j, i], xlabel=xlabel[j], ylabel=ylabels[i, j], xlabelsize = 24, ylabelsize= 24, xticklabelsize=16, yticklabelsize=16, xtickalign = 1, xticksize=4, ytickalign=1, yticksize=4, xgridvisible = false, ygridvisible = false) for i in 1:3 for j in 1:2], 2, 3)
	...
	# for (iq, quark) in enumerate(quarks)
		for iQs in 1:length(Qss)
			...
			for i in 1:3
				# xlims!(axes[1, i], 0, 10)
				# axes[1, i].xticks = ([0, 2.5, 5, 7.5, 10], ["0", "2.5", "5", "7.5", "10"])
				xlims!(axes[1, i], 0, 8)
				axes[1, i].xticks = ([0, 2, 4, 6, 8], ["0", "2", "4", "6", "8"])
			end
			...
			mom_broad_T_wong = mom_broad_wong[tag_wong][:, 2]
			
			lines!(axes[1, 1], δτ_wong, mom_broad_L_wong, linewidth = 1.5, color=(colors[iQs], alpha))
			# scatter!(axes[1], δτ_sel[tag_wong], mom_broad_L_wong[ind_sel[tag_wong]], color=colors[iQs], marker=markers[iq], markersize=7)
			# lines!(axes[1, 1], δτ_sel[tag_wong], mom_broad_L_wong[ind_sel[tag_wong]], linewidth = 1.5, color=(colors[iQs], alpha))
			
			lines!(axes[1, 2], δτ_wong, mom_broad_T_wong, linewidth = 1.5, color=(colors[iQs], alpha))
			# scatter!(axes[2], δτ_sel[tag_wong], mom_broad_T_wong[ind_sel[tag_wong]], color=colors[iQs], marker=markers[iq], markersize=7)
			# lines!(axes[1, 2], δτ_sel[tag_wong], mom_broad_T_wong[ind_sel[tag_wong]], linewidth = 1.5, color=(colors[iQs], alpha))
			
			
			ratio = mom_broad_L_wong./mom_broad_T_wong
			lines!(axes[1, 3], δτ_wong, ratio, linewidth = 1.5, color=(colors[iQs], alpha))
			# ratio = mom_broad_L_wong[ind_sel[tag_wong]]./mom_broad_T_wong[ind_sel[tag_wong]]
			# lines!(axes[1, 3], δτ_sel[tag_wong], ratio, linewidth = 1.5, color=(colors[iQs], alpha))

			# scatter!(axes[3], δτ_sel[tag_wong], ratio[ind_sel[tag_wong]], color=colors[iQs], marker=markers[iq], markersize=7)
			...
			lines!(axes[2, 1], δτ_wong, mom_broad_L_wong, linewidth = 1.5, color=(colors[iQs], alpha))
			lines!(axes[2, 2], δτ_wong, mom_broad_T_wong, linewidth = 1.5, color=(colors[iQs], alpha))
			ratio = mom_broad_L_wong./mom_broad_T_wong
			lines!(axes[2, 3], δτ_wong, ratio, linewidth = 1.5, color=(colors[iQs], alpha))

			for i in 1:3
				xlims!(axes[2, i], 0, 50)
				# axes[2, i].xticks = ([0, 2.5, 5, 7.5, 10], ["0", "2.5", "5", "7.5", "10"])
			end
		end
	# end
	...
	cbar = Colorbar(fig[1:2, 4], limits = (1, 6), colormap =  cgrad(colors, 5, categorical = true), size = 24, labelsize = 24, width = 10, flipaxis = true,
	ticksize=3, tickalign = 0, ticklabelsize = 16, height = Relative(1), label=L"Q_s\,\mathrm{[GeV]}")
	cbar.ticks = ([1.5, 2.5, 3.5, 4.5, 5.5],  Qss)

	if saveplots
		save("plots/mom_broad_Qs_"*quarks[iq]*"_layout.png", fig, px_per_unit = 5.0) 
	end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "ca6d41d2a7a7726298d3ad0566ffb2085534c8ff"
