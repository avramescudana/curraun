...
begin
	...
	fig = Figure(size = (1000, 600), font = "CMU Serif")
	...
	titles = [L"p^x\rightarrow\infty", L"m=5\,\mathrm{GeV}", L"p^x=5\,\mathrm{GeV}"]
	ylabels = [L"\langle\delta p^2_y\rangle\,\mathrm{[GeV^2]}", L"\langle\delta p^2_z\rangle\,\mathrm{[GeV^2]}"]
	...
	axes = [Axis(fig[j, i], 
		...
		ylabel=ylabels[j], xlabelsize = 24, ylabelsize= 24, xticklabelsize=16, yticklabelsize=16, xtickalign = 1, xticksize=4, ytickalign=1, yticksize=4, title= j == 1 ? titles[i] : "", titlesize=24
	# ) for i in 1:3, j in 1:3]
	...
	for (iq, quark) in enumerate(quarks)
		...
		for (i, q2) in enumerate(q2s)
			...
			lines!(axes[iq, 1], τ, (δp²[:,2]), color=segmented_cmap[i], linewidth=custom_linewidth)
	...
	for i in 1:3
		...
		for j in 1:2
			...
			linkyaxes!(axes[1, j], axes[2, j], axes[3, j])
		end
	end
	...
	cbar = Colorbar(fig[1:2, 4], limits = (1, length(q2s)+1), colormap = segmented_cmap, size = 25, labelsize = 24, width = 10, flipaxis = true, ticksize=3, tickalign = 0, ticklabelsize = 14, height = Relative(1), label=L"C_2")
...
begin
	...
	ylabels_scaled = [L"\langle\delta p^2_y\rangle/C_2\,\mathrm{[GeV^2]}", L"\langle\delta p^2_z\rangle/C_2\,\mathrm{[GeV^2]}"]
	fig_scaled = Figure(size = (1000, 600), font = "CMU Serif")
	axes_scaled = [Axis(fig_scaled[j, i], 
		# xlabel=L"\delta\tau\,\mathrm{[fm/}c\mathrm{]}", 
		xlabel=L"\tau\,\mathrm{[fm/}c\mathrm{]}", 
		ylabel=ylabels_scaled[j], xlabelsize = 24, ylabelsize= 24, xticklabelsize=16, yticklabelsize=16, xtickalign = 1, xticksize=4, ytickalign=1, yticksize=4, title= j == 1 ? titles[i] : "", titlesize=24
	# ) for i in 1:3, j in 1:3]
	) for i in 1:3, j in 1:2]
	...
	for (iq, quark) in enumerate(quarks)
		...
		for (i, q2) in enumerate(q2s)
			...
			lines!(axes_scaled[iq, 1], τ, (δp²[:,2]), color=segmented_cmap[i], linewidth=custom_linewidth)
	...
	for i in 1:3
		...
		for j in 1:2
			...
			axes_scaled[i, j].xticks = ([0, 0.5, 1, 1.5, 2], ["0", "0.5", "1", "1.5", "2"])
			# if j!=3
			if j!=2
			...
			linkyaxes!(axes_scaled[1, j], axes_scaled[2, j], axes_scaled[3, j])
		end
	end
	...
	cbar_scaled = Colorbar(fig_scaled[1:2, 4], limits = (1, length(q2s)+1), colormap = segmented_cmap, size = 25, labelsize = 24, width = 10, flipaxis = true, ticksize=3, tickalign = 0, ticklabelsize = 14, height = Relative(1), label=L"C_2")
...
begin
	...
	fig_cs = Figure(size = (1000, 350), font = "CMU Serif")
	# titles = [L"m\rightarrow\infty", L"m=m_\mathrm{beauty}", L"m=m_\mathrm{charm}"]
	axes_cs = [Axis(fig_cs[1, i], 
		# xlabel=L"\delta\tau\,\mathrm{[fm/}c\mathrm{]}", ylabel=L"\langle\delta p^2\rangle_F/\langle\delta p^2\rangle_A", 
		xlabel=L"\tau\,\mathrm{[fm/}c\mathrm{]}", ylabel=L"(\langle\delta p^2\rangle/C_2)_F/(\langle\delta p^2\rangle/C_2)_A", 
		xlabelsize = 24, ylabelsize= 24, xticklabelsize=16, yticklabelsize=16, xtickalign = 1, xticksize=4, ytickalign=1, yticksize=4, title=titles[i], titlesize=24
	) for i in 1:3]
	...
	for (iq, quark) in enumerate(quarks)
		...
		lines!(axes_cs[iq], τ, (δp²_fund[:,2])./(δp²_adj[:,2])./C2_ratio, color=segmented_cmap[5], linewidth=2)
		# lines!(axes_cs[iq], τ, δp²_fund[:,3]./δp²_adj[:,3], color=segmented_cmap[13], linewidth=2)
		lines!(axes_cs[iq], τ, δp²_fund[:,3]./δp²_adj[:,3]./C2_ratio, color=segmented_cmap[13], linewidth=2)

		# ylims!(axes_cs[iq], 0, 2*4/9)
		ylims!(axes_cs[iq], 0.7, 1.3)
		xlims!(axes_cs[iq], 0, 1.5)
		axes_cs[iq].xticks = ([0, 0.5, 1, 1.5, 2], ["0", "0.5", "1", "1.5", "2"])
		...
		lines!(axes_cs[iq], τ, ones(length(τ)), color=:gray, linewidth=2, linestyle=:dash)
		lines!(axes_cs[iq], τ, ones(length(τ)), color=(:gray, 0.4), linewidth=2)
	...
	axislegend(axes_cs[1], elements, labels_legend, position = :lt, labelsize=20, titlesize=20, orientation = :horizontal, framevisible=false)
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "63f4be8f517694f677dc5b869874ca930c1761b5"
