...
# ╔═╡ 570584f2-4125-4d84-861c-79a9301f8219
begin
...
# ╔═╡ 76a35b46-aa8d-4dc0-b95c-5490310621f3
using Colors

# ╔═╡ eca8a110-218d-11ef-226b-7bed68bd1348
md"### Import packages"

# ╔═╡ cae411b2-f87a-43c5-86ef-ee451492dfe5
md"---
### Read $(p_x, p_y)$ from Glasma files"

# ╔═╡ 658a293d-a572-49e4-a62a-8e91e8e2c3d0
begin
	current_path = pwd()
	# filename_glasma = current_path * "/gaussian_broadening_charm_Qs_2.0.pickle"
	filename_glasma = current_path * "/gaussian_broadening_px_py.pickle"
	results_glasma = Pickle.npyload(filename_glasma)

	pTs = results_glasma["pTs"]
	px, py = results_glasma["px"], results_glasma["py"]
	tau = results_glasma["tau"]
end

# ╔═╡ 3ec5d855-e652-46af-8a99-25bd655775f7
begin
	px_τ, py_τ = px[end, :], py[end, :]

	npoint = 1024

	kde_pxpy = kde((px_τ, py_τ), npoints=(npoint, npoint), bandwidth=(0.2,0.2))
end

# ╔═╡ 2e470741-71f3-4a13-ac4b-419af7999750
begin
	cmap = cgrad(:candy, 16, categorical=true)
	...
	transparent_white = RGBA{Float64}(1.0, 1.0, 1.0, 0.0)
	
	# Append the transparent white color to the list twice
	push!(cmap_list, transparent_white)
	
	cmap_transp = cgrad(cmap_list)
end

# ╔═╡ 43e632c6-d91a-454b-ab28-e8c0f6acc0da
begin
	...
	limits = ((-4, 4), (-4, 4), (nothing, nothing)),
	ylabel=L"p_x\,\mathrm{[GeV]}", xlabel=L"p_y\,\mathrm{[GeV]}", zlabel="",
	...
	surface!(ax, kde_pxpy.x, kde_pxpy.y, kde_pxpy.density, colormap=Reverse(cmap_transp), shading=NoShading)
	# hidezdecorations!(ax)

	zlims!(0, findmax(kde_pxpy.density)[1])
	cbar = Colorbar(fig[1, 1], colormap=Reverse(cmap_transp), size = 25, labelsize = 24, width = 12, flipaxis = false, flip_vertical_label = true, ticksize=0, tickalign = 0, ticklabelsize = 18, height = Relative(0.8), label=L"\mathrm{d}^2N/\mathrm{d}p_x\mathrm{d}p_y", labelrotation = pi/2
	)
	# cbar.limits = (0, 0.4)
	# cbar.ticks = ([0, 0.25, 0.5, 0.75, 1],  ["0", "0.25", "0.5", "0.75", "1"])
	cbar.ticks = ([0, 0.5, 1],  ["0", "0.5", "1"])
	
	# cbar.ticks = ([1.5, 2.5, 3.5, 4.5, 5.5],  string.(reverse(pTs)))

	save("plots/final_gaussianbroadpxpy_charm_tau_0.6.png", fig, px_per_unit = 10.0)
	
	fig
end

# ╔═╡ 09f5732b-a6b1-4843-8fd5-d8cee89f63d0
begin
	filename_glasma_pT = current_path * "/gaussian_broadening_pTs_fund.pickle"
	results_glasma_pT = Pickle.npyload(filename_glasma_pT)

	pT_values = results_glasma_pT["pTs"]
	pTs_glasma = results_glasma_pT["pTs_tau"]
	tau_pTs = results_glasma_pT["tau"]

	parameters = results_glasma_pT["parameters"]
end

# ╔═╡ d62fdff9-8545-428d-b08f-5051780511ac
pTs_glasma

# ╔═╡ a2d021f6-bda7-4f89-bc13-9023a905223d
τₛ = 0.1

# ╔═╡ b0856ec0-c855-457e-888b-fd73e897d610
function findminindex(value, array)
	return findmin(element->abs(element-value),array)[2]
end 

# ╔═╡ 5296fe8f-99a1-414c-b5ed-3b07cb9c82ec
τᵢ = findminindex(τₛ, tau_pTs)

# ╔═╡ 2076c054-deec-4d68-a617-79d1b41069ee
begin
	npoint_pTs = 2048
	
	collect_pTs, collect_dndpT = Dict(), Dict()
	for (i,ipT) in enumerate(string.(pT_values))

		if i>1
			kde_ipT = kde(pTs_glasma[ipT][10, :], npoints=npoint, bandwidth=0.7)
		else
			kde_ipT = kde(pTs_glasma[ipT][τᵢ, :], npoints=npoint, bandwidth=0.07)
		end

		# kde_ipT = kde(pTs_glasma[ipT][τᵢ, :], npoints=npoint)
		collect_dndpT[ipT] = kde_ipT.density
		collect_pTs[ipT] = convert(Vector{Float64}, kde_ipT.x)
	end
end

# ╔═╡ 39548e2f-ab43-49af-afba-2f120ad1d894
begin
...
# ╔═╡ c71dd500-edaa-42c8-a73b-038c0f62bdd3
begin
	...
	set_aog_theme!(fonts = (; regular = "CMU Serif"))
	# set_theme!(fonts = (; regular ="CMU Serif"))
	fig_dens = Figure(size = (350, 300), font = "CMU Serif")
	ax_dens = Axis(fig_dens[1,1], xlabel=L"p_T\,\mathrm{[GeV]}", ylabel=L"\mathrm{d}N/\mathrm{d}p_T", xlabelsize = 20, ylabelsize= 22, xticklabelsize=17, yticklabelsize=17, xtickalign = 1, xticksize=5, ytickalign=1, yticksize=5,) 

	Ntp = parameters["NTP"]
	Ntpᵢ = range(0, Ntp, Ntp)

	for (i,ipT) in enumerate(string.(pT_values[1:4]))
		lines!(ax_dens,collect_pTs[ipT], collect_dndpT[ipT], color=colors[i], linewidth=1.5)
		# band!(ax_dens, collect_pTs[ipT], zeros(length(collect_pTs[ipT])), collect_dndpT[ipT], color=(colors[i], 0.15))

		# string_as_varname("elem_"*string(i), [PolyElement(color = (colors[i], 0.1),strokewidth = 0), LineElement(color = colors[i])])
		string_as_varname("elem_"*string(i), [LineElement(color = colors[i])])

		pT_spectra₀ = ones(Ntp).*pT_values[i]
		# lines!(ax_dens, pT_spectra₀, Ntpᵢ./Ntp, color=colors[i], linewidth=2)
		lines!(ax_dens, pT_spectra₀, Ntpᵢ./Ntp*1, color=(colors[i], 0.2), linewidth=1.5)
		lines!(ax_dens, pT_spectra₀, Ntpᵢ./Ntp*1, color=colors[i], linewidth=1.5, linestyle=:dash)
	end

	# density!(pTs_glasma["0"][end, :], color = (:red, 0.3), strokecolor = :red,  bandwidth = 1)

	τₛ_label = [L"0", L"3", L"6", L"9"]
	xlims!(ax_dens, -0.5, 13.5)
	ax_dens.xticks = ([0, 3, 6, 9], [L"0", L"3", L"6", L"9"])
	ylims!(ax_dens, 0, 1.2)
	ax_dens.yticks = ([0, 0.5, 1], [L"0", L"0.5", L"1"])

	# legendlabel = L"p_T\,(\tau_\mathrm{form})\,\mathrm{[GeV]}"
	legendlabel = L"p_T\,(\tau_\mathrm{form})"
	axislegend(ax_dens, [elem_1, elem_2, elem_3, elem_4], τₛ_label, legendlabel, labelsize=15, titlesize=18, position = :rt, orientation =:vertical, backgroundcolor = (:white), framecolor=(:grey80, 0))

	save("plots/final_clean_dNdpT_tau_dep.png", fig_dens, px_per_unit = 10)
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "bfbe0723611aed743891d2659b955e6b4c0be7d6"
