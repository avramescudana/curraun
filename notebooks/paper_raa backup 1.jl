...
begin
	...
	for quark in quarks
		...
		for τₛ in τₛ_values
			dumb_pTs_raa[quark][string(τₛ)], raa_avg[quark][string(τₛ)] = compute_raa_events(τₛ, quark, Qₛ, energy, pdf_type, 2)
		end
	end
...
begin
	...
	for ie in range(1, length(energies))
		...
		dumb_pTs_raa_fonll[label], raa_avg_fonll[label] = compute_raa_events(τₛ_fonll, quark, Qₛ, energies[ie], pdf_types[ie], 50)
	end
