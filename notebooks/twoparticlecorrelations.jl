...
begin
	...
	axis = (width = 300, height = 300, xlabel=L"\Delta\eta", ylabel=L"\Delta\phi",
		xlabelsize = 20, ylabelsize = 20, yticks = ([0, π/2, π, 3*π/2, 2*π], ["0", L"\frac{\pi}{2}", L"\pi", L"\frac{3\pi}{2}", L"2\pi"]), limits = (-4, 4, 0, 2*π))
	dNdΔηdΔϕ = data(df) * mapping(:Δηᵢ, :Δϕᵢ)
	plt = dNdΔηdΔϕ * AlgebraOfGraphics.density(npoints=10^3) * visual(Heatmap, colormap = reverse(cgrad(:beach)))
end
...
begin
	...
	save("plots/Cdetadphi_heatmap_toy_charm_pT_2.png", fig, px_per_unit = 5)
	fig
end
...
begin
	dNdΔηdΔϕ3D = data(df) * mapping(:Δηᵢ, :Δϕᵢ) *
		    AlgebraOfGraphics.density(npoints=10^2) * visual(Surface, shading=false, colormap = reverse(cgrad(:beach)))
	# fig = Figure()
	axis3D = (width = 450, height = 400,
		type=Axis3, limits=(nothing, nothing, (nothing, nothing)), 
		elevation=0.1π, azimuth=0.4π, 
		xlabel=L"\Delta\eta", ylabel=L"\Delta\phi", zlabel=L"\mathcal{C}(\Delta\phi,\Delta\eta)", xlabelsize = 20, ylabelsize = 20, zlabelsize = 20,
		yticks = ([0, π, 2*π], ["0", "π", "2π"])
	)
	# colorbar!(fontsize=20)
end

# ╔═╡ f31d439d-362e-42f2-9f33-eb8f38892ba0
draw(dNdΔηdΔϕ3D, axis=axis3D)
